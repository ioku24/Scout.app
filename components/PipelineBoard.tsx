...
const PipelineBoard: React.FC<PipelineBoardProps> = ({ state, onUpdateStage, onSelectDeal }) => {
  ...
  return (
    <div className="flex overflow-x-auto pb-10 space-x-6 min-h-[calc(100vh-250px)] animate-fade-in scrollbar-hide">
      {stages.map((stage) => {
              ...
              {stageDeals.map(deal => {
                ...
                return (
                  <div 
                    ...
                    className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm hover:border-blue-600/40 dark:hover:border-blue-500/40 hover:shadow-xl hover:shadow-slate-900/5 dark:hover:shadow-black/20 hover:-translate-y-1 transition-all cursor-pointer group/card"
                  >
